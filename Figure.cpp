...
bool Figure::move(bool whitePlayer, std::pair<int, int> startPos,
		std::pair<int, int> endPos, Board& boardObj) {
	...
	case 'P':
		...
		returnStatement = movePawn(whitePlayer, false, startPos, endPos,
				boardObj);
		break;
...
bool Figure::isYour(std::pair<int, int> endPos, Board& board) {
	if (!board.isEmpty(endPos)) {	//Figure fig(board.getChessPiece(endPos));
...
bool Figure::moveKing(bool check, std::pair<int, int> startPos,
		std::pair<int, int> endPos, Board& board) {

	if (abs(endPos.first - startPos.first) <= 1
			&& abs(endPos.second - startPos.second) <= 1) {
		return true;
	} else {
...
bool Figure::moveRook(bool check, std::pair<int, int> startPos,
		std::pair<int, int> endPos, Board& boardObj) {
...
bool Figure::movePawn(bool whitePlayer, bool check,
	...
	if (startPos.second == endPos.second) {
		if (((endPos.first - startPos.first) < 0 && whitePlayer)
				|| ((startPos.first - endPos.first) < 0 && !whitePlayer)) {
			if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
					&& !check)
				cout
						<< "To nie jest poprawny ruch pionka. Pionek nie porusza sie do tylu."
						<< endl;
			return false;
		}
		...
		if (abs(endPos.first - startPos.first) == 2) {
			if ((player && startPos.first != 1)
					|| (!player && startPos.first != 6)) {
				if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
						&& !check)
					cout
							<< "Ruch z o dwa pola mozliwy tylko z wyjsciowej pozycji."
							<< endl;
				return false;
			}
			...
			if (board[(endPos.first + startPos.first) / 2][endPos.second]
				...
				if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
						&& !check)
					cout
							<< "To nie jest poprawny ruch pionka. Brak wolnej drogi."
							<< endl;
				return false;
			}
			...
			if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
					&& !check)
				cout
						<< "To nie jest poprawny ruch pionka. Wykonaj ruch o jedno pole."
						<< endl;
			return false;
		...
		if ((abs(startPos.second - endPos.second) > 1)) {
			...
			if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
					&& !check)
				cout
						<< "To nie jest poprawny ruch pionka. Pionek rusza sie do przodu."
						<< endl;
			return false;
		} else if ((abs(startPos.second - endPos.second) == 1)
				&& ((abs(endPos.first - startPos.first)) > 1)) {
			return false;
		}

		else if ((abs(startPos.second - endPos.second) == 1)
				&& (board[endPos.first][endPos.second] != '.')
				&& (startPos.first - endPos.first) == 0) {
			if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
					&& !check)
		...
		else if (((endPos.first - startPos.first) < 0 && player)
				|| ((startPos.first - endPos.first) < 0 && !player)) {
			if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
					&& !check)
				cout
						<< "To nie jest poprawny ruch pionka. Pionek nie porusza siê do tylu."
						<< endl;
			return false;
		}
	}
...
bool Figure::moveKnight(bool check, std::pair<int, int> startPos,
		std::pair<int, int> endPos, Board& boardObj) {

	if ((abs(endPos.second - startPos.second) != 2
			|| abs(endPos.first - startPos.first) != 1)
			&& (abs(endPos.first - startPos.first) != 2
					|| abs(endPos.second - startPos.second) != 1)) {
		if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
				&& !check)
...
bool Figure::moveBishop(bool check, std::pair<int, int> startPos,
		std::pair<int, int> endPos, Board& boardObj) {
	...
	if (abs(endPos.first - startPos.first)
			!= abs(endPos.second - startPos.second)) {
		if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
				&& !check)
...
bool Figure::moveQueen(bool check, std::pair<int, int> startPos,
		std::pair<int, int> endPos, Board& boardObj) {

	if (moveRook(check, startPos, endPos, boardObj)
			|| moveBishop(check, startPos, endPos, boardObj)) {
		return true;
	} else {
		if ((boardObj.getComputerPlayer() || boardObj.getTwoPlayers())
				&& !check)
...
std::pair<int, int> Figure::anythingGoThere(bool withoutKing, bool whitePlayer,
		std::pair<int, int> position, Board& boardObj) {
	char* tempTable;
	boardObj.getBoard(&tempTable);
	char (*board)[BOARD_SIZE] =
			reinterpret_cast<char (*)[BOARD_SIZE]>(tempTable);
	...
	for (int x = 1; x < 9; x++) {
		for (int y = 0; y < 8; y++) {
			...
			if (board[x - 1][y] != '.') {
				...
				case 'p':
					if (!whitePlayer)
						possibleCheck = movePawn(whitePlayer, true,
								piecePosition, position, boardObj);
					break;
				case 'P':
					if (whitePlayer)
						possibleCheck = movePawn(whitePlayer, true,
				...
				case 'r':
					if (!whitePlayer)
						possibleCheck = moveRook(true, piecePosition, position,
								boardObj);
					break;
				case 'R':
					if (whitePlayer)
				...
				case 'n':
					if (!whitePlayer)
						possibleCheck = moveKnight(true, piecePosition,
								position, boardObj);
					break;
				case 'N':
					if (whitePlayer)
				...
				case 'b':
					if (!whitePlayer)
						possibleCheck = moveBishop(true, piecePosition,
								position, boardObj);
					break;
				case 'B':
					if (whitePlayer)
				...
				case 'q':
					if (!whitePlayer)
						possibleCheck = moveQueen(true, piecePosition, position,
								boardObj);
					break;
				case 'Q':
					if (whitePlayer)
				...
				case 'k':
					if (!withoutKing) {
						if (!whitePlayer)
							possibleCheck = moveKing(true, piecePosition,
									position, boardObj);
					}
					break;
				case 'K':
...
bool Figure::isItCheckMate(bool whitePlayer,
		std::pair<int, int> checkingPosition, Board& boardObj) {
	std::pair<int, int> kingPosition;
	if (whitePlayer) {
		kingPosition = findPiece('k', boardObj);
	} else {
	...
	if (checkingPosition.first == 200) {
		...
		std::pair<int, int> pos = anythingGoThere(false, !whitePlayer,
				checkingPosition, boardObj);
		...
		coveringByRQ = kingCoveringByRQ(whitePlayer, checkingPosition,
				boardObj);
		coveringByBQ = kingCoveringByBQ(whitePlayer, checkingPosition,
				boardObj);
...
//! 1) - Czy krol ma gdzie uciec?
bool Figure::kingEscape(bool whitePlayer, std::pair<int, int> kingPosition,
		Board& boardObj) {
	...
	for (int x = (kingPosition.first - 1); x <= (kingPosition.first + 1); x++) {
		if (x >= 0 && x <= 7) {
			for (int y = (kingPosition.second - 1);
					y <= (kingPosition.second + 1); y++) {
				if (y >= 0 && y <= 7) {
					if (board[x][y] == '.') {
						//Trzeba ukryc krola!
						...
						escapePos.first = x;
						escapePos.second = y;
						possibleCheckMate = anythingGoThere(false, whitePlayer,
								escapePos, boardObj);
...
bool Figure::kingCoveringByRQ(bool whitePlayer,
		std::pair<int, int> checkPosition, Board& boardObj) {
		...
		if (checkPosition.first == kingPosition.first) //w tym samym wierszu
			...
			if (difference > 0) //figura szachujacca na prawo od krola
				...
				for (int i = difference; i > 0; i--) {
					...
					if (anythingGoThere(true, !whitePlayer, temporaryPos,
							boardObj).first != 100)
						return false;
				}
...
bool Figure::kingCoveringByBQ(bool whitePlayer,
		std::pair<int, int> checkPosition, Board& boardObj) {
