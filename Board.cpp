...
void Board::init(char board[BOARD_SIZE][BOARD_SIZE]) {
	...
	const char figuresRow[BOARD_SIZE] =
			{ 'R', 'N', 'B', 'Q', 'K', 'B', 'N', 'R' };
...
bool Board::checkMove(std::string &positions, Board &boardObj) {
	...
	if (quantityOfCharacters.length() != 4) {
		if (getComputerPlayer() || getTwoPlayers()) {
			if (quantityOfCharacters.length() == 3
					|| quantityOfCharacters.length() == 2) {
				cout << "Wprowadziles " << quantityOfCharacters.length()
						<< " znaki typu litera lub cyfra, powinny byc 4!";
			} else if (quantityOfCharacters.length() == 0) {
				cout << "Wprowadziles " << quantityOfCharacters.length()
						<< " liter i cyfr, powinny byc 4!";
			} else if (quantityOfCharacters.length() == 1) {
				cout << "Wprowadziles " << quantityOfCharacters.length()
						<< " litere lub cyfre, powinny byc 4!";
			} else {
				cout << "Wprowadziles " << quantityOfCharacters.length()
						<< " znakow typu litera lub cyfra, powinny byc tylko 4!";
			}
		}
	...
	for (size_t i = 0; i < quantityOfCharacters.length(); i++) {
		if (i % 2 == 0) {
			if ((atoi(&quantityOfCharacters[i])) == 0) {
				...
				if (!((castedChar >= 'A' && castedChar <= 'A' + BOARD_SIZE)
						|| (castedChar >= 'a' && castedChar <= 'a' + BOARD_SIZE))) // 65 == A
				{
					if (getComputerPlayer() || getTwoPlayers())
						cout << "Litera na pozycji " << i + 1
								<< " powinna byc z zakresu A-H lub a-h!"
								<< endl;
					checkChar = false;
				}
				...
				if (getComputerPlayer() || getTwoPlayers())
					cout << "Znak " << i + 1
							<< " powinien byc litera a jest cyfra!" << endl;
				moveCorrectness = false;
			}

		} else {
			if ((atoi(&quantityOfCharacters[i]) < 1)
					|| (atoi(&quantityOfCharacters[i]) > 8)) {
				if (getComputerPlayer() || getTwoPlayers())
					cout << "Liczba na pozycji " << i + 1
							<< " powinna byc z zakresu 1-8!" << endl;
				checkChar = false;
			}
		}
	}
...
bool Board::isAllowed(std::pair<int, int> endPos,
		char board[BOARD_SIZE][BOARD_SIZE], bool player) {

	if (player) {
		if ((board[endPos.first][endPos.second] >= 'A')
				&& (board[endPos.first][endPos.second] <= 'Z')) {
...
bool Board::checkFigure(string& startPosition, string& endPosition,
		string& command, Board& board, bool& player, char& exit) {
...
void Board::move(std::pair<int, int> startPos, std::pair<int, int> endPos,
		Board &boardObj, bool &player) {
	...
	if (checkPlayer(startPos, board, player)
			&& (isAllowed(endPos, board, copyPlayer))) {
		player = !player;
...
bool Board::checkPlayer(std::pair<int, int> startPos,
		char board[BOARD_SIZE][BOARD_SIZE], bool& player) {

	if (player) {
		if ((board[startPos.first][startPos.second] >= 'A')
				&& (board[startPos.first][startPos.second] <= 'Z')) {
...
void Board::move(std::string start, std::string end, Board& boardObj,
		bool &player) {
